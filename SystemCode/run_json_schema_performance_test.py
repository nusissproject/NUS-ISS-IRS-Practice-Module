...
def performance_test(log_caste_list_path:str):
    ...
    json_validator = DataValidationWithJsonSchema(config_path)
    ...
    for log_case_folder in inspection_log_cases:
        print(f"Running json validation for log case: {log_case_folder}")
        result_path = os.path.join(DATA_FOLDER, log_case_folder, "JSONValidationResult")
        ...
        result_info = json_validator(log_case_folder, img, label_json, result_folder=result_path)
...
if __name__ == "__main__":
    print("Running JSON schema validation performance test for training log cases...")
    performance_test(TRAIN_LOG_CASE_LIST_PATH)
    print("Running JSON schema validation performance test for inspection log cases...")
