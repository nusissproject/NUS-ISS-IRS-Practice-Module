...
class Sam3Image(torch.nn.Module):
    ...
    def _encode_prompt(
        ...
        if encode_text:
            # txt_feat: 32 x B X 256
            # geo_feats: (1+N(# of prompt per image)) X B X 256
            # visual_prompt_embed: None
            # txt masks: 32 X B X 256
            # geo masks: (1+N) X B X 256
            # concatenata along batch dimension for prompts with different concepts (IMPORTANT. But how to pad???)
...
class Sam3CrossImage(Sam3Image):
    """
    Variant of Sam3Image that encodes prompts on a reference image and reuses
    them on a target image (or the same image) without recomputing prompt
    features.
    """
    ...
    def forward_with_prompt(
        ...
        with torch.profiler.record_function("SAM3CrossImage._run_encoder"):
            backbone_out, encoder_out, _ = self._run_encoder(
                backbone_out=backbone_out,
                find_input=find_input,
                prompt=prompt,
        ...
        with torch.profiler.record_function("SAM3CrossImage._run_decoder"):
            out, hs = self._run_decoder(
                memory=out["encoder_hidden_states"],
                pos_embed=encoder_out["pos_embed"],
                src_mask=encoder_out["padding_mask"],
        ...
        with torch.profiler.record_function("SAM3CrossImage._run_segmentation_heads"):
            self._run_segmentation_heads(
                out=out,
                backbone_out=backbone_out,
                img_ids=find_input.img_ids,
    ...
    def forward_grounding_cross_image(
        self,
        reference_backbone_out,
        target_backbone_out,
        reference_find_input,
        target_find_input,
        geometric_prompt: Prompt,
        encoder_extra_kwargs: Optional[Dict] = None,
    ):
        prompt, prompt_mask, reference_backbone_out = self._encode_prompt(
            backbone_out=reference_backbone_out,
            find_input=reference_find_input,
            geometric_prompt=geometric_prompt,
        )
        out = self.forward_with_prompt(
            backbone_out=target_backbone_out,
            find_input=target_find_input,
            prompt=prompt,
            prompt_mask=prompt_mask,
            encoder_extra_kwargs=encoder_extra_kwargs,
        )
        return out, reference_backbone_out
